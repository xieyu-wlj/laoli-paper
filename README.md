...
3. **第三段：框架总体效果、模块对位与验证引出贡献**
   必须以 `To address these ..., we propose ..., which ...` 引入方法。`which` 后用一句话概括整个框架实现的总体效果，不能在这里罗列模块、组成或技术步骤；各模块如何回应前文瓶颈，应放在后续句子中展开。

   推荐形式：

   ```text
   To address these challenges, we propose [method], which [用一句话说明整个框架实现的效果].
   ```
